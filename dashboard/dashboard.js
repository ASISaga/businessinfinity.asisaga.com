import { getApiPath } from '../assets/js/apiRoutes.js';
...
class BoardroomDashboard extends HTMLElement {
  ...
  async triggerAction(actionId, bindings) {
    ...
    const { path, method } = getApiPath('postMcp');
  ...
  renderAdminEndpoints() {
    ...
    dashboardBtn.onclick = async () => {
      const { path, method } = getApiPath('getDashboard');
    ...
    statusBtn.onclick = async () => {
      const { path, method } = getApiPath('getStatus');
      const res = await fetch(path, { method });
      const data = await res.json();

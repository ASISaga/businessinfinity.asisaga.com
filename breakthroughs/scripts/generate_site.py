#!/usr/bin/env python3
"""Generate the Business Infinity static website — The Breakthroughs Index.
...
- ``index.html``               — The Breakthroughs Index hub page
- ``breakthroughs/<slug>.html`` — 10 pillar pages (one per pain category)
...
{body}
<footer>
  <p>&copy; ASI Saga &mdash; Business Infinity &mdash; The Breakthroughs Index</p>
  <p>
    <a href="{SITE_URL}">Home</a> &middot;
    <a href="{SITE_URL}/breakthroughs">Breakthroughs Index</a> &middot;
...
# ── Index Page (The Breakthroughs Index Hub) ───────────────────────────────────────
...
def _render_index() -> str:
    ...
    for cat in PAIN_TAXONOMY:
        ...
        cat_cards.append(f"""
<a href="{SITE_URL}/breakthroughs/{cat['slug']}" class="card" style="text-decoration:none;display:block">
...
<div class="card">
  ...
  <p><strong>2. Find</strong> &mdash; This Breakthroughs Index appears as a Featured Snippet with the legendary diagnostic.</p>
...
"""
    ...
    return _page(
        "The Breakthroughs Index",
        body,
        jsonld_blocks=[render_jsonld_script(kg)],
        description=(
            f"The Breakthroughs Index: {total_q} panic-driven search queries "
        ...
        canonical=f"{SITE_URL}/breakthroughs",
    )
...
<div class="breadcrumb">
  <a href="{SITE_URL}/breakthroughs">The Breakthroughs Index</a> &rsaquo;
  Category {cat['number']}
</div>
...
"""
    ...
    return _page(
        f"{cat['title']} — Breakthroughs Index",
        body,
...
def generate(output_dir: Path) -> None:
    ...
    # Pillar pages
    pillar_dir = output_dir / "breakthroughs"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate the Business Infinity Breakthroughs Index website"
    )
    parser.add_argument(
        "--output",
        default="website",
